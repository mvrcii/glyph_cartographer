...
from glyph.model.data_module import SF_DataModule
...
def get_callbacks(cfg, model_run_dir):
    ...
    if node:
        ...
        filename_metric = f"{{{monitor_metric}:.3f}}"
...
def main(config_path, seed=42, gpu=0, checkpoint=None, resume_with_reload=False):
    ...
    config.label_size = config.patch_size // 4
    ...
    check_wandb_login()
    wandb_logger = WandbLogger(project="glyph", entity="wuesuv")
    ...
    data_module = SF_DataModule(cfg=config, model_run_dir=model_run_dir)
    model = SF_Module(**config_dict)
